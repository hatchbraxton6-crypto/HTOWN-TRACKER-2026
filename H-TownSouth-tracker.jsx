...
const SEED_WEEKS = [
  ...
  {
    id: 6, label: "May Wk 2", dates: "5/4/26 – 5/10/26", month: "May",
    reps: [
      { name: "Reese H",      accounts: 16, acv: 1042 },
      { name: "Hannemann W",  accounts: 15, acv: 1023 },
      { name: "Braxton H",    accounts: 14, acv: 1163 },
      { name: "Kevin J",      accounts: 12, acv: 1006 },
      { name: "Salsabil F",   accounts: 12, acv: 1018 },
      { name: "Anthony V",    accounts: 11, acv: 1029 },
      { name: "Seth T",       accounts: 8,  acv: 1069 },
      { name: "Burkley P",    accounts: 5,  acv: 1155 },
      { name: "Jack B",       accounts: 4,  acv: 1006 },
      { name: "John K",       accounts: 4,  acv: 988  },
      { name: "Easton R",     accounts: 4,  acv: 971  },
      { name: "Scott S",      accounts: 3,  acv: 956  },
      { name: "Iohani W",     accounts: 2,  acv: 1023 },
      { name: "David R",      accounts: 1,  acv: 1053 },
      { name: "John L",       accounts: 1,  acv: 913  },
      { name: "Amari W",      accounts: 1,  acv: 1053 },
      { name: "Alvin S",      accounts: 1,  acv: 993  },
      { name: "Mosiah F",     accounts: 1,  acv: 993  },
    ],
  },
];

const STORAGE_KEY = "htown_south_full_v5";
...
function repSummary(weeks) {
  ...
  weeks.forEach((w) =>
    w.reps.forEach((r) => {
      if (!map[r.name]) map[r.name] = { accounts: 0, revenue: 0, acvSum: 0, weeksActive: 0, bestWeek: 0, bestWeekLabel: "", weekHistory: [] };
      map[r.name].accounts += r.accounts;
      map[r.name].revenue += repRev(r);
      map[r.name].acvSum += r.acv;
      map[r.name].weeksActive += 1;
      if (r.accounts >= map[r.name].bestWeek) { map[r.name].bestWeek = r.accounts; map[r.name].bestWeekLabel = w.label; }
...
// ── Achievement Badges ────────────────────────────────────────────
const BADGES = [
  { id: "club20", label: "20 Club", icon: "🔥", color: "#f5c842", bg: "#2a2000", border: "#6a5000", check: (rep) => rep.bestWeek >= 20 },
  { id: "club15", label: "15 Club", icon: "⚡", color: "#3db557", bg: "#0a2010", border: "#1a5030", check: (rep) => rep.bestWeek >= 15 },
];
...
function RepProfileModal({ rep, photo, allWeeks, onClose, onPhotoUpdate, isSlumping }) {
  ...
  const handlePhoto = (f) => {
    ...
    reader.onload = () => onPhotoUpdate(rep.name, reader.result);
  ...
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={{ ...S.modal, maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        <div style={S.modalHead}>
          <span style={S.modalTitle}>{rep.name}</span>
          <button onClick={onClose} style={S.xBtn}>✕</button>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 16 }}>
          <div
            style={{ width: 88, height: 88, borderRadius: '50%', overflow: 'hidden', cursor: 'pointer', border: '2px solid #2a4a2a', marginBottom: 6 }}
            onClick={() => fileRef.current.click()}
          >
            {photo
              ? <img src={photo} alt={rep.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <div style={{ width: '100%', height: '100%', background: '#1a2a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue',cursive", fontSize: 30, color: '#3db557', letterSpacing: 2 }}>{initials}</div>
            }
          </div>
          <div style={{ fontSize: 10, color: '#333' }}>tap to add photo</div>
          <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handlePhoto(e.target.files[0])} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14 }}>
          {[
            ['Total Accts', rep.accounts, '#3db557'],
            ['Total Rev', fmt$(rep.revenue), '#3db557'],
            ['Best Week', rep.bestWeek, '#f5c842'],
            ['Avg/Wk', rep.avgPerWeek.toFixed(1), '#aaa'],
            ['Avg ACV', fmt$(rep.avgAcv), '#aaa'],
            ['Consistency', rep.weeksActive < 2 ? '—' : `${rep.consistency}/100`, rep.consistency >= 80 ? '#3db557' : rep.consistency >= 60 ? '#c8c820' : rep.consistency >= 40 ? '#e09020' : '#e05252'],
          ].map(([label, val, color]) => (
            <div key={label} style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: 8, padding: '8px 4px', textAlign: 'center' }}>
              <div style={{ fontSize: 8, color: '#444', letterSpacing: 1.5, fontWeight: 700, marginBottom: 4, textTransform: 'uppercase' }}>{label}</div>
              <div style={{ fontSize: 15, fontFamily: "'Bebas Neue',cursive", color, letterSpacing: 1 }}>{val}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'linear-gradient(135deg,#2a2000,#1a1400)', border: '1px solid #6a5000', borderRadius: 12, padding: '14px 16px', marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 9, color: '#776600', letterSpacing: 2, fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>Best Week</div>
            <div style={{ fontSize: 11, color: '#554400', fontWeight: 600 }}>{rep.bestWeekLabel}</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: "'Bebas Neue',cursive", fontSize: 54, color: '#f5c842', letterSpacing: 2, lineHeight: 1 }}>{rep.bestWeek}</div>
            <div style={{ fontSize: 9, color: '#776600', letterSpacing: 2, fontWeight: 700 }}>ACCOUNTS</div>
          </div>
        ...
        {(() => {
          const earned = BADGES.filter(b => b.check(rep));
          if (!earned.length) return null;
          return (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: '#444', letterSpacing: 2, fontWeight: 700, marginBottom: 8, textTransform: 'uppercase' }}>Achievements</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {earned.map(b => (
                  <div key={b.id} style={{ background: b.bg, border: `1px solid ${b.border}`, borderRadius: 20, padding: '6px 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 15 }}>{b.icon}</span>
                    <span style={{ fontFamily: "'Bebas Neue',cursive", fontSize: 15, color: b.color, letterSpacing: 1.5 }}>{b.label}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ fontSize: 10, color: '#444', letterSpacing: 2, fontWeight: 700, marginBottom: 8, textTransform: 'uppercase' }}>Week History</div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {rep.weekHistory.map((wh, j) => {
            const mc = monthColor(wh.label);
            return (
              <div key={j} style={{ background: mc.bg, border: `1px solid ${mc.border}`, borderRadius: 8, padding: '6px 10px', minWidth: 68, textAlign: 'center' }}>
...
export default function App() {
  ...
  const [photos, setPhotos] = useState(() => {
    try { const s = localStorage.getItem(PHOTOS_KEY); return s ? JSON.parse(s) : {}; } catch { return {}; }
  });
  ...
  useEffect(() => { try { localStorage.setItem(PHOTOS_KEY, JSON.stringify(photos)); } catch {} }, [photos]);

  const updatePhoto = (name, dataUrl) => setPhotos(prev => ({ ...prev, [name]: dataUrl }));
              ...
              <table style={S.table}>
                ...
                <tbody>{allSummary.map((rep, i) => (
                  <tr key={rep.name} style={S.tr}>
                    ...
                    <td style={{ ...S.td, fontWeight: 600, cursor: "pointer", color: "#e0e0e0" }} onClick={() => setProfileRep(rep.name)}>{rep.name}</td>
                ...
                <table style={S.table}>
                  ...
                  <tbody>{cw.reps.map((r, i) => (
                    <tr key={r.name} style={S.tr}>
                      ...
                      <td style={{ ...S.td, fontWeight: 600, cursor: "pointer", color: "#e0e0e0" }} onClick={() => setProfileRep(r.name)}>{r.name}</td>
